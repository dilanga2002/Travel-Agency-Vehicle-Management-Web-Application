    ...
    <div class="dashboard-container">
        ...
        <div class="sidebar bg-blue-900 text-white">
            ...
            <div class="p-4 border-b border-blue-800 flex items-center">
                <img class="h-10 w-10 rounded-full mr-3" src="{{ $user->profile_photo ? asset('storage/'.$user->profile_photo) : 'https://ui-avatars.com/api/?name=' . urlencode($user->name) . '&background=random' }}" alt="User avatar">
                <div>
            ...
            <div class="sidebar-menu">
                <ul>
                    ...
                    <li>
                        <a href="{{ route('profile.show') }}" class="text-white bg-blue-700 border-l-4 border-blue-400 flex items-center">
            ...
            <header class="bg-white shadow-md rounded-lg p-4 flex justify-between items-center mb-6">
                ...
                <div class="flex items-center space-x-4">
                    <span class="text-sm text-gray-600 hidden md:inline">
                        Last login: {{ auth()->user()->last_login_at?->diffForHumans() ?? 'First time login' }}
                    </span>
                    <div class="relative">
            ...
            <main class="p-6 bg-white rounded-xl shadow">
                ...
                <div class="max-w-2xl mx-auto">
                    ...
                    <section class="mb-12">
                        ...
                        <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data" class="mt-6 space-y-6">
                            ...
                            <div class="form-field">
                                ...
                                <div class="flex items-center">
                                    <img class="h-16 w-16 rounded-full object-cover" src="{{ auth()->user()->profile_photo ? asset('storage/'.auth()->user()->profile_photo) : 'https://ui-avatars.com/api/?name=' . urlencode(auth()->user()->name) . '&background=random' }}" alt="{{ auth()->user()->name }}">
                            ...
                            <div class="form-field">
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name', auth()->user()->name) }}" class="w-full">
                                @error('name')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="form-field">
                                ...
                                <input type="email" name="email" id="email" value="{{ old('email', auth()->user()->email) }}" class="w-full">
                                @error('email')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="form-field">
                                ...
                                <input type="text" name="phone" id="phone" value="{{ old('phone', auth()->user()->phone) }}" class="w-full">
                                @error('phone')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="form-field">
                                ...
                                <textarea name="address" id="address" class="w-full">{{ old('address', auth()->user()->address) }}</textarea>
                                @error('address')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                    ...
                    <section>
                        ...
                        <form action="{{ route('profile.destroy') }}" method="POST" class="mt-6 space-y-6" onsubmit="return confirm('Are you sure you want to delete your account? This action cannot be undone.');">
                            @csrf
                            @method('DELETE')
