    ...
    <div class="dashboard-container">
        ...
        <div class="sidebar bg-blue-900 text-white">
            ...
            <div class="p-4 border-b border-blue-800 flex items-center">
                <img class="h-10 w-10 rounded-full mr-3" src="{{ $driver->profile_photo ? asset('storage/'.$driver->profile_photo) : 'https://ui-avatars.com/api/?name=' . urlencode($driver->name) . '&background=random' }}" alt="Driver avatar">
                <div>
                    <p class="font-medium">{{ $driver->name }}</p>
                    <p class="text-blue-200 text-xs">{{ $driver->email }}</p>
                </div>
            </div>
            ...
            <div class="sidebar-menu">
                <ul>
                    ...
                    <li>
                        <a href="{{ route('driver.profile.edit') }}" class="text-white bg-blue-700 border-l-4 border-blue-400 flex items-center">
            ...
            <header class="bg-white shadow-md rounded-lg p-4 flex justify-between items-center mb-6">
                    ...
                    <nav class="flex" aria-label="Breadcrumb">
                        <ol class="inline-flex items-center space-x-1 md:space-x-3">
                            <li class="inline-flex items-center">
                                <a href="{{ route('driver.dashboard') }}" class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-blue-600">
                ...
                <div class="flex items-center space-x-4">
                    <span class="text-sm text-gray-600 hidden md:inline">
                        Last login: {{ $driver->last_login_at ? $driver->last_login_at->diffForHumans() : 'First time login' }}
                    </span>
                    <div class="relative">
            ...
            <main class="p-6 bg-white rounded-xl shadow">
                ...
                <div class="max-w-2xl mx-auto">
                    ...
                    <section class="mb-12">
                        ...
                        <form action="{{ route('driver.profile.update') }}" method="POST" enctype="multipart/form-data" class="mt-6 space-y-6">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="form-field">
                                ...
                                <div class="flex items-center">
                                    <img class="h-16 w-16 rounded-full object-cover" src="{{ $driver->profile_photo ? asset('storage/'.$driver->profile_photo) : 'https://ui-avatars.com/api/?name=' . urlencode($driver->name) . '&background=random' }}" alt="{{ $driver->name }}">
                            ...
                            <div class="form-field">
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name', $driver->name) }}" class="w-full">
                                @error('name')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="form-field">
                                ...
                                <input type="email" name="email" id="email" value="{{ old('email', $driver->email) }}" class="w-full">
                                @error('email')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="form-field">
                                ...
                                <input type="text" name="phone" id="phone" value="{{ old('phone', $driver->phone) }}" class="w-full">
                                @error('phone')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="form-field">
                                ...
                                <textarea name="address" id="address" class="w-full">{{ old('address', $driver->address) }}</textarea>
                                @error('address')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="form-field">
                                ...
                                <input type="text" name="license_number" id="license_number" value="{{ old('license_number', $driver->license_number) }}" class="w-full">
                    ...
                    <section>
                        ...
                        <form action="{{ route('driver.profile.update') }}" method="POST" class="mt-6 space-y-6">
                            @csrf
                            @method('PUT')
