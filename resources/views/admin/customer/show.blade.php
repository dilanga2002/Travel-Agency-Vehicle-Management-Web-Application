...
<head>
    ...
    <title>SriTravel - Customer Details</title>
    ...
    <div class="dashboard-container">
        ...
        <div class="sidebar bg-blue-900 text-white">
            ...
            <div class="sidebar-menu">
                <ul>
                    ...
                    <li>
                        <a href="{{ route('admin.customers.index') }}" class="text-white bg-blue-700 border-l-4 border-blue-400 flex items-center">
            ...
            <div class="sidebar-footer p-4">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <a href="{{ route('logout') }}" class="bg-red-500 text-white hover:bg-red-600 flex items-center w-full px-4 py-2 rounded-md text-sm font-medium" onclick="event.preventDefault(); this.closest('form').submit();">
        ...
        <div class="main-content">
            ...
            <header class="bg-white shadow-md rounded-lg p-4 flex justify-between items-center mb-6">
                <h1 class="text-2xl font-semibold font-heading text-gray-800">Customer Details</h1>
            ...
            <main class="p-6 bg-white rounded-xl shadow">
                <div class="content-area">
                    <div class="content-header flex items-center justify-between mb-6">
                        <div>
                            <h2 class="text-lg font-medium text-gray-800">Customer Profile</h2>
                            <p class="text-sm text-gray-500">View customer information</p>
                        </div>
                        <a href="{{ route('admin.customers.index') }}" class="inline-flex items-center px-3 py-1 border border-transparent text-sm font-medium rounded shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition duration-200 hover:scale-105">
                    ...
                    <div class="bg-white rounded-lg shadow-md p-6">
                        <div class="flex items-center mb-6">
                            <img src="{{ $user->profile_photo ? asset('storage/' . $user->profile_photo) : 'https://via.placeholder.com/80' }}" alt="{{ $user->name }}" class="h-20 w-20 rounded-full mr-4">
                            <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Registered On</label>
                                <p class="mt-1 text-sm text-gray-900">{{ $user->created_at->format('M d, Y H:i:s') }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Last Updated</label>
                                <p class="mt-1 text-sm text-gray-900">{{ $user->updated_at->format('M d, Y H:i:s') }}</p>
                            </div>
                        </div>
                        <div class="mt-6 flex justify-end">
                            <form action="{{ route('admin.customers.destroy', $user->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this customer?');">
                                @csrf
                                @method('DELETE')
