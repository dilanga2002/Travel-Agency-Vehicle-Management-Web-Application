        ...
        <div class="sidebar bg-blue-900 text-white">
            ...
            <div class="sidebar-menu">
                <ul>
                    ...
                    <li>
                        <a href="{{ route('dashboard') }}" class="text-white bg-blue-700 border-l-4 border-blue-400 flex items-center">
            ...
            <main class="p-6">
                ...
                <div class="bg-gradient-to-r from-blue-600 to-blue-800 text-white rounded-xl p-6 mb-6 shadow-md transition duration-300 hover:scale-105">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="mt-1 text-blue-100">You have {{ $activeBookingsCount }} active bookings</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="dashboard-card bg-white shadow rounded-xl p-6 transition duration-300 hover:scale-105">
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <p class="text-2xl font-semibold text-gray-900">{{ $totalBookingsCount }}</p>
                            </div>
                        </div>
                    </div>
                    <div class="dashboard-card bg-white shadow rounded-xl p-6 transition duration-300 hover:scale-105">
                ...
                <div class="bg-white shadow rounded-xl p-6 mb-6">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <a href="{{ route('bookings.index') }}" class="text-sm text-blue-600 hover:text-blue-800 flex items-center">
                            ...
                            <div class="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition">
                                <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                                    <div class="flex items-start space-x-4 flex-1">
                                        <div class="flex-shrink-0">
                                            <img class="h-16 w-16 rounded-md object-cover" src="{{ $booking->vehicle->image ? asset('storage/'.$booking->vehicle->image) : asset('images/vehicle-placeholder.jpg') }}" alt="">
                                        </div>
                                        <div>
                                            <h4 class="font-medium text-gray-900">{{ $booking->vehicle->make }} {{ $booking->vehicle->model }}</h4>
                                            <div class="mt-1">
                                    ...
                                    <div class="mt-4 md:mt-0 md:flex md:items-center md:justify-end md:space-x-2">
                                        <a href="{{ route('bookings.show', $booking->id) }}" class="inline-flex items-center px-3 py-1 border border-transparent text-xs font-medium rounded shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                            <i class="fas fa-eye mr-1"></i> View
                                        </a>
                                        @if($booking->status == 'pending')
                                            <button onclick="confirmCancel('{{ $booking->id }}')" class="inline-flex items-center px-3 py-1 border border-transparent text-xs font-medium rounded shadow-sm text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                                                <i class="fas fa-times mr-1"></i> Cancel
                                            </button>
                                            <form id="cancel-booking-{{ $booking->id }}" action="{{ route('bookings.cancel', $booking->id) }}" method="POST" class="hidden">
                                                @csrf
                                                @method('PATCH')
                                            </form>
                                        @endif
                ...
                <div class="bg-white shadow rounded-xl p-6">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <a href="{{ route('vehicles.index') }}" class="text-sm text-blue-600 hover:text-blue-800 flex items-center">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <div class="vehicle-card bg-white border border-gray-200 rounded-lg overflow-hidden transition duration-300 hover:scale-105 shadow-sm">
                            <div class="relative h-48">
                                <img class="absolute h-full w-full object-cover" src="{{ $vehicle->image ? asset('storage/'.$vehicle->image) : asset('images/vehicle-placeholder.jpg') }}" alt="{{ $vehicle->make }} {{ $vehicle->model }}">
                                <div class="absolute top-2 right-2 bg-blue-600 text-white text-xs px-2 py-1 rounded-full">
                                    {{ ucfirst($vehicle->type) }}
                                </div>
                            </div>
                            <div class="p-4">
                                <h4 class="font-bold text-lg text-gray-900">{{ $vehicle->make }} {{ $vehicle->model }}</h4>
                                <div class="flex items-center text-gray-600 text-sm mt-1">
                                    <span class="mr-2">{{ $vehicle->passengers }} passengers</span>
                                    @if(!empty($vehicle->features))
                                        <span class="mr-2">•</span>
                                        <span>{{ implode(' • ', $vehicle->features) }}</span>
                                    @endif
                                </div>
                                <div class="mt-3">
                                    <span class="text-xl font-bold text-blue-600">Rs. {{ number_format($vehicle->price_per_km, 2) }}</span>
                                    <span class="text-gray-500 text-sm"> / 1km</span>
                                </div>
                                <div class="mt-4">
                                    <a href="{{ route('vehicles.show', $vehicle->id) }}" class="block w-full text-center bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded-md transition duration-300 hover:scale-105">
